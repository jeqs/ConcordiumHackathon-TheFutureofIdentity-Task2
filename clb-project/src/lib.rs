...
#[receive(contract = "clb_project", name = "view", return_value = "State")]
...
#[receive(contract = "clb_project", name = "receive", parameter = "bool", error = "Error", mutable)]
fn receive<S: HasStateApi>(
    ...
    let throw_error = ctx.parameter_cursor().get()?; // Returns Error::CustomError on failure
    if throw_error {
...
#[receive(contract = "clb_project", name = "set", parameter = "i32", mutable)]
fn set<S: HasStateApi>(
    ctx: &impl HasReceiveContext,
    _host: &mut impl HasHost<State, StateApiType = S>
) -> Result<(), Error> {
    // Your code

    let throw_error = ctx.parameter_cursor().get()?; // Returns Error::ParseParamsError on failure
    if throw_error {
...
mod tests {
    ...
    fn test_throw_error() {
        ...
        // Check the result.
        claim_eq!(error, Err(Error::CustomError), "Function should throw an error.");
    }
}